...
- 正規公開URL: [https://core-analysis.jp/](https://core-analysis.jp/)
- 公開画面・決済後の受付・PDF・メール: XServer共有サーバー
- 鑑定文・高精細作品の生成: `shop.unfashion8.com` 配下の連携VPS
- Renderは使用しません。`makaren.onrender.com` は停止済みで、本番確認や公開先に使わないでください。

本番反映では、XServer上の現行ファイルを正として先にバックアップし、PHP構文、生成API、作品画像、PDF全ページ、決済画面を検証してから切り替えます。
