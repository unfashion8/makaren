...
- 【人名の表記】文中に登場するすべての人名は、入力で与えられたローマ字表記のまま使用すること。漢字・ひらがな・カタカナへの変換や推測表記は禁止。誤字を防ぐため、必ずローマ字のみで記載すること。
...
- 【名前の表記】本文中で本人の名前を出す場合は、必ず上記「表示名」で与えたローマ字表記のみを使用すること。漢字・ひらがな・カタカナへの変換や推測表記は禁止。誤字を防ぐため、必ずローマ字のまま記載すること。
...
- 【名前の表記（必須）】各人物の名前は、上記「周囲の人物」で与えたローマ字表記をそのまま使用すること。漢字・ひらがな・カタカナへの変換、推測による別表記は禁止。誤字を防ぐため、必ずローマ字のみで記載すること。
